defmodule ExDebugger do
  ...
  defmacro def(def_heading_ast, def_do_block_ast \\ nil) do
    ...
    if Tokenizer.module_has_use_ex_debugger?(tokenizer, __CALLER__.module) do
      ...
      updated_def_do_block_ast = def_do_block_ast
        |> case do
          [do: {:__block__, ctx, statements}] ->
            [last_expression | remainder] = if Tokenizer.bifurcates?(tokenizer) do
            ...
            [
              do: {
                  :__block__,
                  ctx,
                  Enum.reverse(remainder) ++ AstWalker.pipe_debug_expression(last_expression, type: :def_output_only, line: Tokenizer.last_line(tokenizer))
                  }
            ]
          [do: statement = {op, ctx, statements}] ->
            [
              do: {
                  ...
                  |> AstWalker.pipe_debug_expression(type: :def_output_only, line: Tokenizer.last_line(tokenizer))
              }
            ]
          [do: statement] ->
            [
      ...
      Meta.debug(updated_def_do_block_ast, tokenizer.meta_debug, tokenizer.def_name, :show_ast_after)
      quote do
        Kernel.def(
          unquote(def_heading_ast), unquote(updated_def_do_block_ast))
        end

    else
      quote do
        Kernel.def(
          unquote(def_heading_ast), unquote(def_do_block_ast))
        end
    end
  end
    ...
    quote do
      ...
      Kernel.def d(various, label, env \\ "", bindings \\ "", force_output? \\ false) do
        if @global_output || force_output? || @default_output do
          ...
          event = %{piped_value: various, label: label, bindings: bindings, env: ExDebugger.Formatter.env_to_map(env)}
          @capture_medium
          |> case do
