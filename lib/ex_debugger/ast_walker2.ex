defmodule ExDebugger.AstWalker2 do
  ...
  def inc(a = acc(current_counter: current_counter)), do: acc(a, current_counter: current_counter + 1)
  ...
  def incorporate_piped_debug_expressions(expressions, t = %Tokenizer{}) do
    expressions
    |> Macro.postwalk(acc(tokenizer: t), &postwalk/2)
    |> elem(0)
  end

  def incorporate_piped_debug_expression(expressions, t = %Tokenizer{}, type: type, current_line: l, current_counter: i) do
    a = acc(tokenizer: t)
  ...
  def postwalk({:case, ctx = [line: current_line], do_clause}, a = acc()) do
    ...
    |> case do
      ...
      [[do: case_scenarios]] -> {{:case, ctx, [[do: handle_case_scenarios(case_scenarios, acc(a, current_line: current_line))]]}, a}
      # classic case scenario with expression embedded therein
      [embedded_expression, [do: case_scenarios]] -> {{:case, ctx, [embedded_expression, [do: handle_case_scenarios(case_scenarios, acc(a, current_line: current_line))]]}, a}
    end
  end
  def postwalk({operator, ctx, statements}, a = acc()) when is_list(statements) do
    {{operator, ctx, Enum.map(statements, fn e ->
      postwalk(e, a) |> elem(0)
    end)}, a}
  end
  ...
  def handle_block({:__block__, ctx, statements}, acc(tokenizer: t, current_line: l, current_counter: i)) do
    {:__block__, ctx, incorporate_piped_debug_expression(statements, t, type: :case_statement, current_line: l, current_counter: i)}
  end
  def handle_block(statements, acc(tokenizer: t, current_line: l, current_counter: i)) do
    {:__block__, [], incorporate_piped_debug_expression(List.wrap(statements), t, type: :case_statement, current_line: l, current_counter: i)}
  end
end
