defmodule ExDebugger.AstWalker2 do
...

          {
            updated_blocks ++
              [
                {key,
                 {:__block__, ctx,
                  incorporate_piped_debug_statement(
                    statements,
                    ctx,
                    String.to_atom("if_#{key}"),
                    0,
                    count
                  )}}
              ],
  ...
  def handle_bifurcation({:case, ctx, do_clause}) do
    ...
    |> case do
      ...
      [[do: case_scenarios]] ->
        {:case, ctx, [[do: Enum.map(case_scenarios, &handle_case_scenario/1)]]}

      # classic case scenario with expression embedded therein
      [embedded_expression, [do: case_scenarios]] ->
        {:case, ctx,
         [embedded_expression, [do: Enum.map(case_scenarios, &handle_case_scenario/1)]]}
    end
  end
  ...
  def handle_block(statements, parent_ctx) do
    {:__block__, [],
     incorporate_piped_debug_statement(List.wrap(statements), parent_ctx, :case_statement)}
  end

  def incorporate_piped_debug_statement(
        statements,
        parent_ctx,
        statement_juncture,
        offset \\ 0,
