...
In my personal practice of doing `TDD` and getting to the bottom of a particular bug the above options are lacking:
  ...
  * Option 3, to set up manually introduces a great amount of overhead and the `VSCode plugin` never worked for me out of the box. Next, in my `TDD` flow,
  I just want to `mix test some_file.exs:34` and get on with my life.
  * Option 4 and 5 seem attractive to me in terms of debugging something running in
  production and most certainly there is value in becoming versatile in these tools by using them in your development as a practice. But most certainly there is a high upfront cost associated with
  such an exploration and does adopting such liability make sense in your daily `TDD` flow?

In all the above cases, the main stumbling block on my part is that none of them are particularly fun to use in the context of a `TDD`-flow or quickly hacking something together. Debugging requires a certain focus and the shorter you can make the feedback loop the better.

As an experiment, I authored this library with the aim of keeping ceremony to a bear minimum and thus it aims to:
  1. Be so simple it is accessible to every developer regardless as to their seniority level/experience. e.g.:
     1. All the obvious places in which change of state can occur are being annotated with hidden debugging statements to minimize clutter. These are:
        ...
        2. At every juncture inside every bifurcation expression, e.g.:
           * case
           * if/else
           * cond
...
2. VSCode Extension:
   ...
   * Enable/Disable in the walk through view certain `Debugging Events` that were persisted and persist this subset somehow for future replay purposes either in VSCode or either as a GIF. This
   may facilitate in communicating certain mistakes in the code base on Github etc.
